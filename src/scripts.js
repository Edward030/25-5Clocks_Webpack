...
class App extends React.Component {
  ...
  handleBreaktime(ButtonValue) {
    ...
    if (
      !isRunning &&
      times.break + parseInt(ButtonValue) > 0 &&
      times.break + parseInt(ButtonValue) < 61
    ) {
      this.setState({
        times: {
          ...this.state.times,
          ...
          time:
            type === 'break'
              ? times.time + parseInt(ButtonValue) * 60
              : times.time,
        },
      });
    }
  ...
  handleSessiontime(ButtonValue) {
    ...
    if (
      !isRunning &&
      times.session + parseInt(ButtonValue) > 0 &&
      times.session + parseInt(ButtonValue) < 61
    ) {
      this.setState({
        times: {
          ...this.state.times,
      ...
      <div id="panel">
        ...
        <div id="combined-panel">
          <div id="break-panel">
            ...
            <div id="break-length" value={times.break}>
              {times.break}
            </div>
          ...
          <div id="session-panel">
            ...
            <div id="session-length" value={times.session}>
              {times.session}
            </div>
        ...
        <div id="timer-panel">
          ...
          <div
            id="time-left"
            value={`${String(Math.floor(times.time / 60)).padStart(
              2,
              '0'
            )}:${String(times.time % 60).padStart(2, '0')}`}
          >
            {String(Math.floor(times.time / 60)).padStart(2, '0')}:
            {String(times.time % 60).padStart(2, '0')}
          </div>
        </div>

        <div id="play-panel">
          <div id="start_stop" onClick={() => this.timer(1)}>
          ...
          <audio
            id="beep"
            src="https://audio.jukehost.co.uk/hyDuDWJRJaBOUv6n604wVmsfQEivCHme.mp3"
            type="audio"
          ></audio>
        </div>
      </div>
